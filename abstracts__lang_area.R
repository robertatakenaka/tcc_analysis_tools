...
g1 <- ds_g[ds_g$subject_area %in% c('Linguística, Letras e Artes', 'Ciências Sociais Aplicadas'), ]
draw_graphic(g1, "title", paste(GRAPHIC_FILE_PATH, "LLA_CSA", sep=""))
    
g2 <- ds_g[ds_g$subject_area %in% c('Ciências Humanas', 'Ciências da Saúde'), ]
draw_graphic(g2, "title", paste(GRAPHIC_FILE_PATH, "HUM_SAU", sep=""))

g3 <- ds_g[ds_g$subject_area %in% c("Ciências Biológicas", "Ciências Exatas e da Terra", "Engenharias", "Ciências Agrárias"), ]
draw_graphic(g3, "title", paste(GRAPHIC_FILE_PATH, "BEEA", sep=""))
