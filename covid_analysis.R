...
# parece que há pouca correlação entre ter ou não doi e a quantidade de reflinks encontrados
...
# g %>%
#   arrange(desc(n)) %>%
#   mutate(reflinks = factor(reflinks, reflinks)) %>%
#   ggplot(aes(x=n, y=reflinks, size=n, fill=reflinks)) +
#     geom_point(alpha=0.5, shape=21, color="black") +
#     coord_flip() +
#     scale_size(range = c(.1, 24), name="Ocorrências") +
#     scale_fill_viridis(discrete=TRUE, guide=TRUE, option="A") +
#     theme_ipsum() +
#     theme(legend.position="bottom") +
#     ylab("Vínculos") +
#     xlab("Registrado por DOI") +
