...
display_numbers <- function(n, total) {
...
xray <- function(ds_papers, ds_sources) {
    ...
    values <- c(values, display_numbers(sum(ds_sources$doi), total_sources))

    df_refs <- data.frame(labels=labels, values=values)
    df <- rbind(df, df_refs)

    df_sources <- t_sources(ds_sources)
    df <- rbind(df, df_sources)
    ...
    values <- c(values, display_numbers(total_connections - sum(ds_papers$score_none), total_connections))
    ...
    values <- c(values, display_numbers(ge_70, total_connections))
    
    labels <- c(labels, 'Conexões por similaridade semântica >= 90%')
    values <- c(values, display_numbers(sum(ds_papers$score_gt_89), total_connections))

    labels <- c(labels, 'Conexões por similaridade semântica 80-89%')
    values <- c(values, display_numbers(sum(ds_papers$score_gt_79), total_connections))

    labels <- c(labels, 'Conexões por similaridade semântica 70-79%')
    values <- c(values, display_numbers(sum(ds_papers$score_gt_69), total_connections))

    labels <- c(labels, 'Conexões por similaridade semântica 60-69%')
    values <- c(values, display_numbers(sum(ds_papers$score_gt_59), total_connections))

    labels <- c(labels, 'Conexões por similaridade semântica 1-59%')
    values <- c(values, display_numbers(sum(ds_papers$score_gt_1_59), total_connections))

    labels <- c(labels, 'Conexões por similaridade semântica < 1%')
    values <- c(values, display_numbers(sum(ds_papers$score_lt_1), total_connections))
...
connections_papers <- function(ds_papers) {
    report <- ds_papers %>%
        mutate(labels=case_when(
            ...
            connections == 0 ~ 'Artigos não vinculados pelas referências bibliográficas'
        )) %>%
        group_by(labels) %>%
        summarise(n=n()) %>%
...
max_score_ranges_papers <- function(ds_papers) {
    report <- ds_papers %>%
        mutate(max_score_ranges=case_when(
            score_gt_89 > 0 ~ 'Artigos cuja pontuação mais alta é entre 90-100',
            score_gt_79 > 0 ~ 'Artigos cuja pontuação mais alta é entre 80-89',
            score_gt_69 > 0 ~ 'Artigos cuja pontuação mais alta é entre 70-79',
            score_gt_59 > 0 ~ 'Artigos cuja pontuação mais alta é entre 60-69',
            score_gt_1_59 > 0 ~ 'Artigos cuja pontuação mais alta é entre 1-59',
            score_lt_1 > 0 ~ 'Artigos cuja pontuação mais alta é menor que 1',
